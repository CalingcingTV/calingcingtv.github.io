...
//     div.className = 'flex-shrink-0 w-40 bg-gray-800 p-3 rounded-lg cursor-pointer hover:bg-gray-700';
//     // div.className = 'bg-gray-800 p-3 rounded-lg cursor-pointer hover:bg-gray-700';
//     div.innerHTML = `<img src="${ch.logo}" alt="${ch.name}" class="max-w-full h-auto max-h-24 object-contain mb-2" />
...
function renderChannels() {
  ...
  .forEach(ch => {
    const div = document.createElement('div');
    div.className = 'flex-shrink-0 w-64 bg-gray-800 p-3 rounded-lg cursor-pointer hover:bg-gray-700';
    div.innerHTML = `<div class="h-24 flex items-center justify-center mb-2">
    <img src="${ch.logo}" alt="${ch.name}" class="max-w-full max-h-full object-contain" />
    </div>
    <p class="text-center font-medium text-sm truncate">${ch.name}</p>`;
    div.onclick = () => playChannel(ch);
    channelListEl.appendChild(div);
  });
}
